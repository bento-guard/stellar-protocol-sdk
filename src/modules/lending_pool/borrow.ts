...
export async function borrowForAgent(client: BentoStellarClient, action: AgentPoolActionRequest): Promise<any> {
  return postJson(client, `${LENDING_POOL_BASE}/agent/borrow`, action, 'Failed to borrow from agent lending pool');
}
