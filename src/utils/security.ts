...
export async function executeSecureAgentAction(client: RequestClient, config: SecurityActionConfig) {
  ...
  const transaction_id = draftResult?.transaction_id || draftResult?.data?.transaction_id;
  ...
  console.log(`[SDK] ✅ Step 1 Success - Created Draft TxID: ${transaction_id}`);
  ...
  if (body.instruction) {
    ...
    const status = riskResult?.status || riskResult?.data?.decision || riskResult?.data?.status;
    const reason = riskResult?.reason || riskResult?.data?.reasoning || 'Unknown';

    if (status === 'BLOCKED') {
      console.log(`[SDK] ❌ Step 2 Failed - Security Gate Blocked Action: ${reason}`);
    ...
    if (status === 'ESCALATED') {
      console.log(`[SDK] ⚠️ Step 3: Transaction Escalated. Awaiting manual review.`);
      return { status: 'escalated', transaction_id, reason };
    }

    // Step 3: Approve if ALLOWED
    console.log(`[SDK] 🚀 Step 3: Approving Transaction Draft...`);
    const approveResult: any = await approveTransaction(client, { txId: transaction_id });
